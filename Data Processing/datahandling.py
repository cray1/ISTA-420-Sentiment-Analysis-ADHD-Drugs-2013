...
if "_" in filename:
    ...
    drug=filename[:-4] 
tweets=open(filepath+filename,'r').readlines()
...
for line in tweets:
    ...
    print event
    messageArray = { "drug" : drug, "text" : text,"messageId" : messageId,"userId" : userId,"utc_offset" : utc_offset, "time_zone" : time_zone,"lang" : lang, "timestamp" : timestamp, "sentiment" : sentiment, "side_effect" : event}
    if len(text)>0:
